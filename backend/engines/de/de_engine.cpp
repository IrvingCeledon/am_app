...
void DEEngine::perform_iteration() {
    ...
    for (size_t i = 0; i < pop_size; ++i) {
        ...
        do { r1 = idx_dist(rng); } while (r1 == i);
        do { r2 = idx_dist(rng); } while (r2 == i || r2 == r1);
        do { r3 = idx_dist(rng); } while (r3 == i || r3 == r1 || r3 == r2);

        RealIndividual trial(dims);
        ...
        for (size_t j = 0; j < dims; ++j) {
            if (prob(rng) <= this->configuration.CR || j == R) {
                trial.genes()[j] = this->pop[r1].genes()[j] +
                this->configuration.F * (this->pop[r2].genes()[j] - this->pop[r3].genes()[j]);
            } else {
                trial.genes()[j] = this->pop[i].genes()[j];
            }
        }
        ...
        // Greedy selection
        if (trial.getCost() <= this->pop[i].getCost()) {
            next_gen[i] = trial;
        } else {
            next_gen[i] = this->pop[i];
        }
    }
